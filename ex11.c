...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        // Skip processing if input is too long
        if (userInput == NULL) {
            continue;
        }
 ...
 * 
 * Reads characters until newline or EOF, growing the buffer as needed.
 * Returns NULL if input exceeds MAX_INPUT_LENGTH
 */
char* get_string() {
    size_t buffer_size = MAX_INPUT_LENGTH;  // Start with reasonable buffer size
    size_t length = 0;                      // Track current input length
...
int is_dangerous_command(char **user_args, int user_args_len) {
    ...
    return 0;  // Not a dangerous command
}
