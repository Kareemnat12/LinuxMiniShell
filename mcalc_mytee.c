...
int mcalcFlag_left = 0; // Global flag to indicate if mcalc is active
int mcalcFlag_right = 0; // Global flag to indicate if mcalc is active on the right side
/////MONITORING
...
int my_tee_handler(void) {
    ...
    for (int i = 1; i < r_args_len; i++) {
        ...
        flags |= append_flg ? O_APPEND : O_TRUNC;

        int fd = open(r_args[i], flags, 0644);
        if (fd < 0) {
            perror("open");
            continue;
        }
    ...
    // Stream data directly without buffering
    char buffer[4096];
    ssize_t bytes;
...
int pipe_split(char *input, char *left_cmd, char *right_cmd) {
    ...
    if (token) {
        ...
        strncpy(left_cmd, token, MAX_INPUT_LENGTH - 1);
        left_cmd[MAX_INPUT_LENGTH - 1] = '\0'; // Ensure null-termination

        token = strtok(NULL, "|");
        if (token) {
            strncpy(right_cmd, token, MAX_INPUT_LENGTH - 1);
            right_cmd[MAX_INPUT_LENGTH - 1] = '\0';
            result = 1;
        } else {
            right_cmd[0] = '\0';
            result = 0;
